...
import {
  bsioitemmodel, childitem,
  childseedtype,
  customerInfo,
  DeliveryOrderList,
  FullCustomerInfo,
  LineInfo,
} from './createbreederseedissueordermodel';
...
@Component({
    selector: 'app-createbreederseedissueorder',
    templateUrl: './createbreederseedissueorder.component.html',
    styleUrls: ['./createbreederseedissueorder.component.scss']
})
export class CreatebreederseedissueorderComponent implements OnInit {

    displayedColumns: string[] = ['item_no', 'item_name','stage','male_female','item_class_of_seed','variety_group','lot_no','fg_pack_size','no_of_bags','quantity', 'unit_price', 'total_amount', 'crop_code','Action'];
  ...
  GetCropCode(){
    try {
      this.spinner.show()
      this.webApiHttp.Get(this.webApiHttp.ApiURLArray.GetBreederTypeCropCode)
        .then(result => {
  ...
  GetChildSeedType(data,event){
    ...
    try {
      this.spinner.show()
      this.webApiHttp.Get(this.webApiHttp.ApiURLArray.GetChilDSeedType+data)
        .then(result => {
          if (Array.isArray(result) && result.length) {
  ...
  getLine(){
      try{
        this.webApiHttp.Get(this.webApiHttp.ApiURLArray.GetBSIOLine+this.AddItem.get('ItemNo').value+'&location_id='+this.sessionManageMent.getLocationId)
          .then(result=>{
            if(result.length>0 && result[0].condition.toLowerCase()=='true'){
             ...
             // this.dataSource=new MatTableDataSource<LineInfo>(this.LineDetail)
              ...
              this.AddItem.get('ItemNo').setValue(''),
                ...
                this.AddItem.get('malefemale').setValue(''),
  ...
  addnewitem() {
    try {
      if (this.AddItem.get('NoOfBags').value > 0 ) {
        ...
        // let temdata:Array<LineInfo>=[]
        // temdata.push(addlines);
        // if(temdata.length>0) {
        //   for (let i = 0; i < this.dataSource.data.length; i++) {
        //     if (temdata[i].item_no == this.dataSource.data[i].item_no) {
        //       temdata[i].quantity += this.dataSource.data[i].quantity
        //     }
        //   }
        // }
        // console.log(temdata)
  ...
  set_item_on_line(rowelement : any,i : number,item_list_row:any){
    this.dataSource.data[i].lot_no = rowelement.lot_no
  }
  getQty(){
  ...
  CreateSaleOrder() {
    ...
    for(let i =0;i<this.itemdetail.length;i++){
      for(let j = 0;j<this.dataSource.data.length;j++){
        if(this.itemdetail[i].item_no == this.dataSource.data[j].item_no){
    ...
    try {
      ...
      this.webApiHttp.Post(this.webApiHttp.ApiURLArray.BreederSeedOrderCreation, json).then(result => {
        if (result[0].condition.toLowerCase() === 'true') {
