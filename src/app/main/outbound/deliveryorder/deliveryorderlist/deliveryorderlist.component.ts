...
import {Deliveryorderlistmodel} from "./deliveryorderlistmodel";
...
export class DeliveryorderlistComponent implements OnInit {
    ...
    dataSource: MatTableDataSource<Deliveryorderlistmodel>;
    ...
    create_sale_order() {
        this.router.navigateByUrl('/outbound/deliveryordercreate');
    }
    ...
    get_delivery_order() {
        try {
            ...
            this.webApiHttp.Get(this.webApiHttp.ApiURLArray.DeliveryOrderList+this.sessionManageMent.getLocationId).then(result => {
              console.log(result)
                if (result[0].condition == 'True') {
                    this.dataSource = new MatTableDataSource<Deliveryorderlistmodel>(result);
