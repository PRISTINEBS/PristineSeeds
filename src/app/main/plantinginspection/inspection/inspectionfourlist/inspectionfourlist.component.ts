...
@Component({
  selector: 'app-inspectionfourlist',
  templateUrl: './inspectionfourlist.component.html',
  styleUrls: ['./inspectionfourlist.component.scss']
})
export class InspectionfourlistComponent implements OnInit {
  ...
  get_inspection_four_list() {
    try {
      this.spinner.show();
      this.webApiHttp.Get(this.webApiHttp.ApiURLArray.get_inspection_four_list).then(result => {
  ...
  OnClickEdit(rowelement : any){
    this.router.navigate(['/plantinginspection/createinspection4', {inspection_no: this._encryptdecrypt.encrypt(rowelement.inspection_no)}])
  }
