...
@Component({
  selector: 'app-inspectionthreelist',
  templateUrl: './inspectionthreelist.component.html',
  styleUrls: ['./inspectionthreelist.component.scss']
})
export class InspectionthreelistComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.get_inspection_three_list();
  }

  get_inspection_three_list() {
    try {
      this.spinner.show();
      this.webApiHttp.Get(this.webApiHttp.ApiURLArray.get_inspection_three_list).then(result => {
  ...
  OnClickEdit(rowelement : any){
    this.router.navigate(['/plantinginspection/createinspection3', {inspection_no: this._encryptdecrypt.encrypt(rowelement.inspection_no)}])
  }
