...
@Component({
  selector: 'app-inspectiontwolist',
  templateUrl: './inspectiontwolist.component.html',
  styleUrls: ['./inspectiontwolist.component.scss']
})
export class InspectiontwolistComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.get_inspection_two_list();
  }

  get_inspection_two_list() {
    try {
      this.spinner.show();
      this.webApiHttp.Get(this.webApiHttp.ApiURLArray.get_inspection_two_list).then(result => {
  ...
  OnClickEdit(rowelement : any){
    this.router.navigate(['/plantinginspection/createinspection2', {inspection_no: this._encryptdecrypt.encrypt(rowelement.inspection_no)}])
  }
