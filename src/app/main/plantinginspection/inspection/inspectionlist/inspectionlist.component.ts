...
import {Plantinglistmodel} from '../../planting/plantinglist/plantinglistmodel';
...
import {Inspectionlistmodel} from './inspectionlistmodel';

@Component({
  selector: 'app-inspectionlist',
  templateUrl: './inspectionlist.component.html',
  styleUrls: ['./inspectionlist.component.scss']
})
export class InspectionlistComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.get_inspection_one_list();
  }

  get_inspection_one_list() {
    try {
      this.spinner.show();
      this.webApiHttp.Get(this.webApiHttp.ApiURLArray.get_inspection_one_list).then(result => {
  ...
  OnClickEdit(rowelement : any){
    this.router.navigate(['/plantinginspection/createinspection1', {inspection_no: this._encryptdecrypt.encrypt(rowelement.inspection_no)}])
  }
