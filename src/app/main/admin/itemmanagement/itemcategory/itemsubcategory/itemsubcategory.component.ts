...
export class itemsubcategoryComponent implements OnInit {
    ...
    displayedColumns: string[] = ['seed_type','code', 'name', 'description', 'Edit', 'Delete'];
    ...
    ngOnInit(): void {
        ...
        try {
            this.spinner.show();
            this.webApiHttp.Get(this.webApiHttp.ApiURLArray.ItemSubCategoryList + this.data.id)
                .then(result => {
    ...
    OnClickNew() {
        var dialogConfig = this.dialogActions.open(itemsubcategorycreationComponent, {
            data: {flag: 'insert', id: this.data.id, seed_type: this.data.seed_type}
        })
