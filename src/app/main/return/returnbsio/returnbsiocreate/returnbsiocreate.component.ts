...
import {createreturnbsio, get_bsio_details_from_ile, get_document_no_from_ile} from './returnbsiocreatemodel';
import {production_generate} from '../../../plantinginspection/planting/plantingview/plantingviewmodel';

@Component({
  selector: 'app-returnbsiocreate',
  templateUrl: './returnbsiocreate.component.html',
  styleUrls: ['./returnbsiocreate.component.scss']
})
export class ReturnbsiocreateComponent implements OnInit {
  ...
  constructor(
    ...
    this.bsioReturnForm = this._formBuilder.group({
      location: [null, Validators.required],
      CustomerNameorNo: [null, Validators.required],
      bsiono: [null, Validators.required],
    });
  ...
  get_location() {
    try {
      ...
      this.webApiHttp.Get(this.webApiHttp.ApiURLArray.locationlist).then(result => {
        ...
        this.bsioReturnForm.get('location').setValue(parseInt(this.sessionManageMent.getLocationId));
      })
    } catch (e) {
      this.spinner.hide();
      this._toster.error(e, 'Error');
  ...
  get_bsio_no_from_ile(customer : any) {
    try {
      this.spinner.show();
      this.webApiHttp.Get(this.webApiHttp.ApiURLArray.get_bsio_no_from_ile +
  ...
  get_order(){
    ...
    if(this.bsioReturnForm.get('location').value == null
      || this.bsioReturnForm.get('location').value == ''){
      this._toster.warning('Please Select Location', 'Warning');
      return;
    }

    if(this.bsioReturnForm.get('CustomerNameorNo').value == null
      || this.bsioReturnForm.get('CustomerNameorNo').value == ''){
      this._toster.warning('Please Select Customer', 'Warning');
      return;
    }

    if(this.bsioReturnForm.get('bsiono').value == null
      || this.bsioReturnForm.get('bsiono').value == ''){
      this._toster.warning('Please Select BSIO No', 'Warning');
      return;
    }

    try {
      this.spinner.show();
      this.webApiHttp.Get(this.webApiHttp.ApiURLArray.get_bsio_details_from_ile +
        this.bsioReturnForm.get('bsiono').value).then(result => {
          if (result[0].condition.toLowerCase() == 'true') {
  ...
  create_return_bsio(){
    ...
    const posted_json = {
      bsio_no: this.bsioReturnForm.get('bsiono').value,
      customer_no: this.bsioReturnForm.get('CustomerNameorNo').value,
      location_id: this.bsioReturnForm.get('location').value,
      created_by: this.sessionManageMent.getEmail,
      lines: create_return
    };

    this.webApiHttp.Post(this.webApiHttp.ApiURLArray.create_bsio_return, posted_json).then(
      result => {
        if (result[0].condition.toLowerCase() == 'true') {
          this.spinner.hide();
          this._toster.success(result[0].message, 'Success');
          this.router.navigateByUrl('/return/returnbsiolist');
        } else {
