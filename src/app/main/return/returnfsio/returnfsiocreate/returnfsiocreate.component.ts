...
@Component({
  selector: 'app-returnfsiocreate',
  templateUrl: './returnfsiocreate.component.html',
  styleUrls: ['./returnfsiocreate.component.scss']
})
export class ReturnfsiocreateComponent implements OnInit {
  ...
  constructor(
    ...
    this.fsioReturnForm = this._formBuilder.group({
      location: [null, Validators.required],
      CustomerNameorNo: [null, Validators.required],
      fsiono: [null, Validators.required],
    });
  ...
  get_location() {
    try {
      ...
      this.webApiHttp.Get(this.webApiHttp.ApiURLArray.locationlist).then(result => {
        ...
        this.fsioReturnForm.get('location').setValue(parseInt(this.sessionManageMent.getLocationId));
      })
    } catch (e) {
      this.spinner.hide();
      this._toster.error(e, 'Error');
  ...
  get_bsio_no_from_ile(customer : any) {
    try {
      this.spinner.show();
      this.webApiHttp.Get(this.webApiHttp.ApiURLArray.get_fsio_no_from_ile +
  ...
  get_order(){
    ...
    if(this.fsioReturnForm.get('location').value == null
      || this.fsioReturnForm.get('location').value == ''){
      this._toster.warning('Please Select Location', 'Warning');
      return;
    }

    if(this.fsioReturnForm.get('CustomerNameorNo').value == null
      || this.fsioReturnForm.get('CustomerNameorNo').value == ''){
      this._toster.warning('Please Select Customer', 'Warning');
      return;
    }

    if(this.fsioReturnForm.get('fsiono').value == null
      || this.fsioReturnForm.get('fsiono').value == ''){
      this._toster.warning('Please Select FSIO No', 'Warning');
      return;
    }

    try {
      this.spinner.show();
      this.webApiHttp.Get(this.webApiHttp.ApiURLArray.get_fsio_details_from_ile +
        this.fsioReturnForm.get('fsiono').value).then(result => {
        if (result[0].condition.toLowerCase() == 'true') {
  ...
  create_return_fsio(){
    ...
    const posted_json = {
      fsio_no: this.fsioReturnForm.get('fsiono').value,
      customer_no: this.fsioReturnForm.get('CustomerNameorNo').value,
      location_id: this.fsioReturnForm.get('location').value,
      created_by: this.sessionManageMent.getEmail,
      lines: create_return
    };

    this.webApiHttp.Post(this.webApiHttp.ApiURLArray.create_fsio_return, posted_json).then(
      result => {
        if (result[0].condition.toLowerCase() == 'true') {
          this.spinner.hide();
          this._toster.success(result[0].message, 'Success');
          this.router.navigateByUrl('/return/returnfsiolist');
        } else {
